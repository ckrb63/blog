...
디바이스 캘린더에서 스케쥴을 가져오면, 이어지는 스케쥴은 해당하는 모든 날짜에 대해서 전부 가져오게됩니다. 이때 event.startDate와 비교하여 같은 날짜만 스케쥴 컴포넌트를 렌더링합니다. 예를들어, 4일 기간의
...
```ts
const adjustEventIndexForMonthTransition = (eventEndDate, eventStartDate, startIndex, endIndex) => {
...
const populateCalendarWithEvents = (events, updatedCalendar, firstDayOfMonth) => {
  events.forEach((event) => {
    ...
    adjustEventIndexForMonthTransition(eventEndDate, date, &startIndex, &endIndex, updatedCalendar);
