...
일반적으로 Javascript 코드를 작성할 때 코드 최상단에 import로 모든 모듈을 가져옵니다. 이는 빌드 시점에 모듈 번들러에 의해 의존성 트리에 포함되고 트리에 있는 모든 모듈을 포함하는 하나의 번들 파일이 생성됩니다.
그러므로 어플리케이션의 크기가 커질 수록, 특정 기능에서 필요한 모듈을 사용하기도 전에 전부 불러오다보니 번들의 크기가 계속 커지고 브라우저가 이를 내려받는 시간은 길어집니다. 
다행이도 Vite나 Next.js를 사용하면 이런 문제를 비교적 쉽게 해결할 수 있습니다. 이 글은 Vite를 기준으로 모듈을 나눠서 불러오는 방법에 대해 다룹니다.

## 라이브러리 코드 스플리팅
Vite에서는 config파일의 rollupOptions를 조금 수정하면 쉽게 외부 라이브러리에 대한 청크를 분리할 수 있습니다. 해당 라이브러리가 사용될 때 청크 파일을 불러오고 그 후로는 캐싱하여 사용합니다.
```ts
...
사실 zustand같은 전역 상태관리 라이브러리의 경우에는 거의 모든 페이지에서 사용되고 있습니다. 이러한 경우에는 vendor 청크에 포함시켜 항상 로드되도록 하는 것이 HTTP 요청의 횟수를 줄일 수 있기 때문에 유리할 수 있다고 생각합니다.
