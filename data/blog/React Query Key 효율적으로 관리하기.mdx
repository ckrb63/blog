...
draft: false
summary: '리액트 쿼리 만든사람이 추천하는 쿼리 키 관리하는 방법'
---

![keys](/static/images/blog/keys.jpg)

React Query는 React 프로젝트에서 서버 상태 관리를 용이하게 해주는 잘 알려진 강력한 라이브러리입니다. 그 중 Query Key(이하 쿼리 키)는 React Query에서 매우 중요한 개념입니다.
이 글은 쿼리 키의 특성과 어떻게 하면 효과적으로 관리할 수 있는지, 그리고 실제 프로젝트에 적용시킨 사례에 대해 다룹니다.
...
# 쿼리 키의 관리
이런 쿼리 키를 매번 수동으로 선언하는 것은 오류가 발생하기 쉽고, 중복되기 쉬우며, 일관성이 없어집니다.
...
조금만 더 신경써서, 쿼리 키 마다 JSDoc을 사용해서 어떤 queryFn과 매칭되는지 알려주기로 했습니다. 재사용할 때 해당 쿼리 키가 어떤 역할을 하는지 좀 더 분명해질 것 같습니다.
```ts
/** getWardRequestList */
requestList: ({ wardId, teamId, year, month }: { wardId: number; teamId: number; year: number; month: number; }) =>
  [...wardKeys.base, 'requests', wardId, teamId, year, month] as const,
...
매개변수를 처리하는 방식이나 JSDoc을 사용하는 방식은 제가 이것을 저의 프로젝트에 적용하면서 추가한 방식이고, 여러분들도 알맞게 커스텀한 후 저에게 더 좋은 방법을 알려주시면 감사하겠습니다.